...
// CORS Configuration: Allow only localhost and enable credentials
const corsOptions = {
...
// Block requests with internal cloud metadata IP
...
app.post('/login', async (req, res) => {
    try {
        ...
        const token = jwt.sign(payload, keys.secretKey, { expiresIn: '2h' });

        // Return token and user data in the response
        res.cookie('token', token, { httpOnly: true, sameSite: 'Lax' });  // Set token in cookie
        res.status(200).json({
            success: true,
            token: `Bearer ${token}`,  // Token in response for non-cookie-based authentication
            user
        });
    } catch (err) {
